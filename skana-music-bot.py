...
def finalize_logging(mID):
    fname = "logs/musicbot.%s.log" % (mID,)
    if os.path.isfile(fname):
        log.info("Moving old musicbot log")
        try:
            if os.path.isfile(fname + ".last"):
                os.unlink(fname + ".last")
            os.rename(fname, fname + ".last")
        except:
            pass
...
# TODO: all of this
def sanity_checks(mID, optional=True):
...
def main():
    ...
    if len(sys.argv) < 2:
        print('You forgot to indicate the music-bot ID (first argument)')
        return
    mID = sys.argv[1]
    os.environ["mID"] = mID
    # print(mID)
    ...
    if len(sys.argv) < 3:
        print('You forgot to indicate the music-bot token (second argument)')
        return
    token = sys.argv[2]    
