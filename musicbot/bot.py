...
class MusicBot(discord.Client):
    def __init__(self, mID=None, token=None, config_file=None, perms_file=None, aliases_file=None):
        try:
        ...
        if mID is not None:
            self.mID = mID                
            self.config.command_prefix = self.mID + self.config.command_prefix                    
        ...
        self.downloader = downloader.Downloader(download_folder='audio_cache/' + self.mID)
    ...
    def _setup_logging(self):
        ...
        if self.config.debug_mode:
            ...
            dhandler = logging.FileHandler(filename='logs/discord.%s.log' % (mID), encoding='utf-8', mode='w')
    ...
    async def update_now_playing_status(self, entry=None, is_paused=False):
        ...
        if not self.config.status_message:
            ...
            game = discord.Activity(type=4, name=self.config.status_message.strip()[:128])
...
#######################################################################################################################
    ...
    def run(self):
        try:
            ...
            # Add if token, else
            print(self.config._login_token)
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        #e.set_footer(text='skana-music-bot ({})'.format(BOTVERSION), icon_url='https://i.imgur.com/gFHBoZA.png')
